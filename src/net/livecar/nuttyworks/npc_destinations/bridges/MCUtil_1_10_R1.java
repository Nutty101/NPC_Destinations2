...
public class MCUtil_1_10_R1 implements MCUtilsBridge {
	...
	public boolean isLocationWalkable(Location l, Boolean openGates, Boolean openWoodDoors, Boolean openMetalDoors) {
		...
		if (b.getType().toString().contains("SLAB") || b.getType().toString().contains("STEP")) {
			if (b.getData() < 8) {
				...
				if (b.getRelative(0, 2, 0).getType().toString().contains("SLAB")
						|| b.getRelative(0, 2, 0).getType().toString().contains("STEP"))
	...
	public SLABTYPE getSlabType(Block block) {
		if (block.getType() == Material.DOUBLE_STEP || block.getType() == Material.DOUBLE_STONE_SLAB2
