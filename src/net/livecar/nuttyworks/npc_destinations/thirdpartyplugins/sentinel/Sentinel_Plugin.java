...
public class Sentinel_Plugin {
    ...
    public Sentinel_LocationSetting getCurrentSettings(NPC npc) {
        ...
        if ((version[0] == 0 && version[1] > 4) || (version[0] >= 1)) {
            ...
            for (SentinelTarget target : sentTrait.targets)
                sentStorage.targets.add(target.toString());

            sentStorage.ignores = new ArrayList<String>();
            for (SentinelTarget ignore : sentTrait.ignores)
                sentStorage.ignores.add(ignore.toString());
    ...
    public void setCurrentSettings(NPC npc, Sentinel_LocationSetting sentStorage) {
        ...
        if ((version[0] == 0 && version[1] > 4) || (version[0] >= 1)) {
            ...
            sentTrait.ignores = new HashSet<SentinelTarget>();
            for (String name : sentStorage.ignores)
                sentTrait.ignores.add(SentinelTarget.valueOf(name));

            sentTrait.targets = new HashSet<SentinelTarget>();
            for (String name : sentStorage.targets)
                sentTrait.targets.add(SentinelTarget.valueOf(name));
